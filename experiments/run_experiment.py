...
def main():
    ...
    # 特征提取 - 如果是声音特征，需要传递 metadata
    if args.feature_type == 'sound':
        X_train = extractor.extract(X_train_raw, fs=12000, rpm=rpms_train, metadata=meta_train)
        X_test = extractor.extract(X_test_raw, fs=12000, rpm=rpms_test, metadata=meta_test)
    else:
